...
public class MainApp {
	    ...
	    while (true) {
	        ...
	        switch (opcion2) {
	            case "1":
	                ...
	                for (Producto producto : List.of(producto1, producto2, producto3, producto4, producto5,
	                ...
	                // Verificar si se encontró el producto en el catálogo y si está disponible en la cantidad solicitada
	                if (productoEnCatalogo != null && productoEnCatalogo.getCantidadDisponible() >=cantidadAgregar) {
	                    // Agregar una nueva instancia del producto al carrito con la cantidad especificada
	                    ...
	                    JOptionPane.showMessageDialog(null, "Se han añadido " + cantidadAgregar + " unidades de " + nombreProducto + " al carrito");
	...
	private static void eliminarProductos(List<Producto> carrito) {
	    ...
	    int indexToDelete;
	    try {
	        indexToDelete = Integer.parseInt(JOptionPane.showInputDialog(null, sb.toString() +
	...
	private static void verCesta(List<Producto> carrito) {
	    ...
	    for (Producto producto : carrito) {
	        sb.append("- ").append(producto.getNombre()).append(": ").append(producto.getCantidadDisponible()).append(" unidad(es)\n");
	    }
	...
	private static void realizarCompra(List<Producto> carrito) {
	    ...
	    for (Producto producto : carrito) {
	        ...
	        sb.append("- ").append(producto.getNombre()).append(": ").append(producto.getCantidadDisponible()).append(" unidad(es) - Precio con IVA: ").append(precioConIva).append("€\n");
	    }
